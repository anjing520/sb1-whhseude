...
export default function Header({ onQuote }: HeaderProps) {
  ...
  const navClass = (extra: string) =>
    `flex items-center gap-1 px-4 py-2 rounded-lg text-sm font-semibold transition-colors cursor-pointer ${extra}`;
  ...
  const mobileLinkClass = 'block px-3 py-2.5 rounded-lg text-base font-semibold text-navy-800 hover:bg-navy-50';
    ...
    <header className="fixed top-0 inset-x-0 z-50">
      ...
      >
        <div className="mx-auto max-w-7xl px-6 flex items-center justify-between h-20">
          <a href="#home" onClick={handleNavClick('#home')} className="flex items-center gap-2.5 group cursor-pointer">
            ...
            <span
              className={`text-xl font-extrabold tracking-tight transition-colors ${
                scrolled || !onHome ? 'text-navy-900' : 'text-white'
              }`}
            >
              ...
              if (isRoute) {
                return (
                  <a
                    key={item.label}
                    href={item.href}
                    className={navClass(desktopLinkClass)}
                  >
                    {item.label}
                  </a>
                );
                      ...
                      <div className="rounded-xl bg-white shadow-xl shadow-navy-950/10 ring-1 ring-navy-100 p-2">
                        {item.children.map((child) => (
                          <a
                            ...
                            className="block px-3 py-2.5 rounded-lg text-sm font-medium text-navy-700 hover:bg-navy-50 hover:text-navy-950 transition-colors cursor-pointer"
                          >
                            {child.label}
                          </a>
                        ))}
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`lg:hidden p-2 rounded-lg transition-colors ${
                scrolled || !onHome ? 'text-navy-900 hover:bg-navy-50' : 'text-white hover:bg-white/10'
              }`}
            ...
            {NAV.map((item) => (
              <div key={item.label}>
                {item.href.startsWith('/') ? (
                  <a href={item.href} onClick={() => setMobileOpen(false)} className={mobileLinkClass}>
                    {item.label}
                  </a>
                ) : (
                  <a href={item.href} onClick={handleNavClick(item.href)} className={mobileLinkClass}>
                    {item.label}
                  </a>
                )}
                {item.children && (
